function [Phi,timeVal,error_list] = C1_FEM_CGM(mesh,N_,plot_logistics)   
...
Edge_Mat_byNode = zeros(Num_Nodes,Num_Nodes);
for e=1:Num_Elements
    ne(1) = mesh.Elements(1,e);
    ne(2) = mesh.Elements(2,e);
    ne(3) = mesh.Elements(3,e);
    Edge_Mat_byNode(ne(1),ne(2))=1;
    Edge_Mat_byNode(ne(1),ne(3))=1;
    Edge_Mat_byNode(ne(2),ne(3))=1;
end
...
Elements_Domain_SubDomain1 = findElements(mesh,'region','Face',[4,5,6,7]);
% Find the elements associated with subdomain 2 
Elements_Domain_SubDomain2 = findElements(mesh,'region','Face',[3,5,7,8]);
% Find the elements associated with subdomain 3 
Elements_Domain_SubDomain3 = findElements(mesh,'region','Face',[2,5,8,9]);
% Find the elements associated with subdomain 4 
Elements_Domain_SubDomain4 = findElements(mesh,'region','Face',[1,4,5,9]);
...
if plot_logistics
    ...
    hold on
    pdemesh(mesh.Nodes,mesh.Elements(:,Elements_Domain_SubDomain1),'EdgeColor','blue')
    pdemesh(mesh.Nodes,mesh.Elements(:,Elements_Domain_SubDomain2),'EdgeColor','magenta')
    pdemesh(mesh.Nodes,mesh.Elements(:,Elements_Domain_SubDomain4),'EdgeColor','green')
    pdemesh(mesh.Nodes,mesh.Elements(:,Elements_Domain_SubDomain3),'EdgeColor','red')
    set(gca,'fontsize',24);
    xlim([-6,6])
    ylim([-6,6])
    hold off;
